...
export class FactHandler {
  ...
  public async handleFact(fact: Fact): Promise<AnomalyResult> {
    ...
    try {
      ...
      if (results.events.length > 0) {
        ...
        this.logger.log(
          `✅ No anomalies detected for ${fact.source} (${processingTime}ms)`
        );
      }
      ...
      const processingTime = Date.now() - startTime;
      console.error(
        `❌ Error processing fact from ${fact?.source || "unknown"}:`,
        {
  ...
  private async handleAnomalies(result: AnomalyResult): Promise<void> {
    ...
    this.logger.log(
      `🚨 ${anomalies.length} anomalies detected for ${fact.source} (${processingTimeMs}ms):`,
      anomalies.map((event) => ({
        type: event.type,
        params: event.params,
      }))
    ...
    for (const anomaly of anomalies) {
      try {
        ...
        this.logger.log(
          `✅ Alert created and saved: ${alert.alert_id} for anomaly ${anomaly.type} from ${fact.source}`
        );
        ...
        this.logger.error(
          `❌ Failed to create/save alert for anomaly ${anomaly.type}:`,
          {
            error: error instanceof Error ? error.message : String(error),
            fact: this.sanitizeFactForLogging(fact),
            anomaly: {
  ...
  private generateAlertDescription(fact: Fact, anomaly: Event): string {
    ...
    switch (anomaly.type) {
      case "high-error-rate":
        return `High error rate detected from ${fact.source}: ${
          params.count || "N/A"
        } errors in ${params.timeWindow || "unknown"} timeframe (${
          params.rate || "N/A"
        }% error rate)`;

      case "potential-scraper":
        return `Potential web scraping detected from ${fact.source}: ${
          params.requestCount || "N/A"
        } requests to ${
          params.uniqueUrls || "N/A"
        } unique URLs in rapid succession`;

      case "suspicious-activity":
        return `Suspicious activity pattern detected from ${fact.source}: ${
          params.pattern || "Unknown pattern"
        }. ${params.description || "No additional details"}`;

      case "unauthorized-access":
        return `Unauthorized access attempts detected: ${
          params.attempts || "N/A"
        } failed authentication attempts for ${"unknown user"} from ${
          fact.source
        }`;

      case "failed-syscalls":
        return `System call failures detected from ${fact.source}: ${
          params.failureType || "Unknown type"
        } failures in critical system operations`;

      case "silence-detected":
        return `Service silence detected: No activity from ${fact.source} for ${
          params.silenceDuration || "extended"
        } period, indicating potential service failure`;

      case "repeated-errors":
        return `Repeated error pattern from ${
          fact.source
        }: Same error message occurred ${
          params.count || "multiple"
        } times, indicating systemic issue`;

      default:
        return `Anomaly detected from ${fact.source}: ${
          anomaly.type
  ...
  private generateSuggestedAction(anomaly: Event): string {
    const actionMap: Record<string, string> = {
      "high-error-rate":
        "Investigate error logs, check system resources, review recent deployments",
      "potential-attack":
        "Block suspicious IPs, review security logs, activate incident response",
      "suspicious-activity":
        "Monitor closely, review user behavior, consider security assessment",
      "unauthorized-access":
        "Block source IP, review authentication logs, reset affected credentials",
      "security-violation":
        "Immediate security review, check access controls, audit user permissions",
      "potential-scraper":
        "Implement rate limiting, consider IP blocking, review bot protection",
      "failed-syscalls":
        "Check system health, review resource availability, investigate infrastructure",
      "silence-detected":
        "Check service health, verify connectivity, investigate potential outage",
      "repeated-errors":
        "Review application logs, check configuration, investigate root cause",
    };

    return (
      actionMap[anomaly.type] ||
      "Review anomaly details and investigate further"
    );
  }
  ...
  private async processAnomaly(fact: Fact, anomaly: Event): Promise<void> {
    ...
    this.logger.log(
      `📤 Processing anomaly: ${anomaly.type} for ${fact.source}`
    );
